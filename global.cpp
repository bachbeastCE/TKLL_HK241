...
void receive_db(){
        ...
        //RECEIVE RELAY MODE
        Firebase.RTDB.getString(&fbdo, "/Relay/mode");
        String RelayMode = fbdo.stringData();
        if (RelayMode == "Auto") {
            if (getLightlevel() < 200) {        //Auto control Relay
                for (int i = 0; i < MAX_RELAY; i++) {
                    if (relays[i].status != 0) {
                        relays[i].turnRelayON();
                        relays[i].status = 1;
                    }
                }
            } else if (getLightlevel() > 800) {
                for (int i = 0; i < MAX_RELAY; i++) {
                    if (relays[i].status != 1) {
                        relays[i].turnRelayOFF();
                        relays[i].status = 0;
                    }
                }
            }     
